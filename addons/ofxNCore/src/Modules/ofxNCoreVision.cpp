...
void ofxNCoreVision::loadXMLSettings()
{
	...
	in_socket_port				= atoi(XML.getValue("CONFIG:IN_SOCKET:IN_SOCKET_PORT", "3998").c_str());
	std::string xml_source		= XML.getValue("CONFIG:SOURCE","VIDEO");
	bcamera	= (xml_source == "MULTIPLEXER");
	bvideo	= (xml_source == "VIDEO");		
	bsocket	= (xml_source == "IN_SOCKET");
//original line
...
void ofxNCoreVision::saveSettings()
{
	...
	//socket video input support
	if(bcamera)
		XML.setValue("CONFIG:SOURCE", "MULTIPLEXER");
	else if(bsocket)
		XML.setValue("CONFIG:SOURCE", "IN_SOCKET");
	else if(bvideo)
		XML.setValue("CONFIG:SOURCE", "VIDEO");
//Original line
...
void ofxNCoreVision::initDevice()
{
	...
	else
	{
		...
		else if (bsocket)
		{
			...
			//wait until the first correct frame
			while(!sockStreamer->isFrameNew());
				
			printf("Socket Streaming Mode\n\n");
			camHeight = static_cast<unsigned int>(sockStreamer->getHeight());
			camWidth = static_cast<unsigned int>(sockStreamer->getWidth());
			return;
		}
	}
}
...
void ofxNCoreVision::_update(ofEventArgs &e)
{
	...
	else 
	{
		...
		else if(bsocket) //is socket stream
		{
			//socket video input support
			if(sockStreamer != NULL )
				bNewFrame = sockStreamer->isFrameNew();
		}
		else
			bNewFrame = false;
	}
...
void ofxNCoreVision::getPixels()
{
	...
	{
		if(bvideo)	//Get pixels from video
		{
			if(vidPlayer != NULL )
			{
		...
		else if(bsocket)
		{
			...
			if(sockStreamer != NULL )
			{
				...
				sourceImg.setFromPixels(sockStreamer->getPixels(), camWidth, camHeight);
				processedImg = sourceImg;
				if(contourFinder.bTrackFiducials || bFidtrackInterface){processedImg_fiducial = sourceImg;}
			}
		}
	...
	else
	{
		...
		else if(bsocket)
		{
			...
			if(sockStreamer != NULL )
			{
				...
				glEnable(GL_TEXTURE_2D);
				glBindTexture(GL_TEXTURE_2D, target);
				glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, camWidth, camHeight, GL_RGB, GL_UNSIGNED_BYTE, sockStreamer->getPixels());
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
				glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
				glBindTexture(GL_TEXTURE_2D,0);
			}
