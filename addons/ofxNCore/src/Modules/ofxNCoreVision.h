...
public:
	ofxNCoreVision(bool debug)
	{
		...
		bcamera = false;
		bvideo = false;
		bsocket = false;		//socket video input support
		sockStreamer = NULL;	//socket video input support
		bWinTouch = 0;
	...
	ofxSocketStreamer* sockStreamer;	//socket video input support
	...
	bool				bcamera;	//this tree variables are exclusive
	bool				bvideo;		//this tree variables are exclusive
	bool				bsocket;	//this tree variables are exclusive	//socket video input support
    int					deviceID;
	int 				frameseq;
	int 				threshold;
	int					wobbleThreshold;
